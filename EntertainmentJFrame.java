...
public class EntertainmentJFrame extends JFrame implements MouseListener {
    ...
    public EntertainmentJFrame(String username) {
    ...
    private void initView() {
        ...
        // 添加背景
        JLabel background = new JLabel(new ImageIcon(classLoader.getResource("background/娱乐界面背景.png")));
    ...
    public void mouseClicked(MouseEvent e) {
        if (e.getSource() == submitLabel) {
            System.out.println("娱乐费用提交按钮被按下");
            saveEntertainmentData();
        }
    }
    ...
    private void saveEntertainmentData() {
        try {
            String amountText = amountField.getText();
            String paymentMethod = paymentMethodField.getText();
            String time = timeField.getText();
            String remarks = remarksField.getText();
            // 改为使用类中已保存的currentUser（构造方法传入的用户名）
            // String currentUser = UserSession.getCurrentUser(); // 注释掉这行
            String currentUser = this.currentUser; // 使用实例变量

            // 输入验证
            if (amountText.isEmpty() || paymentMethod.isEmpty() || time.isEmpty() || currentUser == null) {
                JOptionPane.showMessageDialog(this, "请填写必要信息或用户信息错误！");
                return;
            }
            ...
            String sql = "INSERT INTO entertainment (Amount, payment_method, time, remarks, User_name) VALUES (?, ?, ?, ?, ?)";
            PreparedStatement pstmt = conn.prepareStatement(sql);
            pstmt.setDouble(1, amount);
            pstmt.setString(2, paymentMethod);
            pstmt.setString(3, time);
            pstmt.setString(4, remarks);
            pstmt.setString(5, currentUser); // 此时currentUser应为非空

            int rows = pstmt.executeUpdate();
            if (rows > 0) {
                JOptionPane.showMessageDialog(this, "娱乐费用记录成功！");
                this.dispose();
            }
            pstmt.close();
            conn.close();
