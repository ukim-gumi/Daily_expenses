...
public class FoodBeverageJFrame extends JFrame implements MouseListener {
    ...
    public FoodBeverageJFrame(String username) {
    ...
    private void initView() {
        ...
        amountLabel.setBounds(0, 95, 150, 50);
        this.getContentPane().add(amountLabel);

        amountField.setBounds(140, 110, 200, 25);
        this.getContentPane().add(amountField);

        // 地点标签和输入框
        JLabel LocationLabel = new JLabel(new ImageIcon(classLoader.getResource("Record_expenses/地点.png")));
        LocationLabel.setBounds(0, 135, 150, 50);
        this.getContentPane().add(LocationLabel);

        LocationField.setBounds(140, 150, 200, 25);
        this.getContentPane().add(LocationField);
        ...
        // 添加背景
        JLabel background = new JLabel(new ImageIcon(classLoader.getResource("background/食品餐饮界面背景.png")));
    ...
    public void mouseClicked(MouseEvent e) {
        ...
        if (e.getSource() == submitLabel) {
            System.out.println("食品餐饮提交按钮被点击");
    ...
    private void saveWaterElectricityData() {
        try {
            ...
            String sql = "INSERT INTO food_beverage (Amount, Location,payment_method, time, remarks, User_name) VALUES (?, ?, ?, ?, ?,?)";
            PreparedStatement pstmt = conn.prepareStatement(sql);
            pstmt.setDouble(1, amount);
            pstmt.setDouble(2, Double.parseDouble(Location));
            ...
            if (rows > 0) {
                JOptionPane.showMessageDialog(this, "食品餐饮记录成功！");
                this.dispose(); // 关闭窗口
            }
